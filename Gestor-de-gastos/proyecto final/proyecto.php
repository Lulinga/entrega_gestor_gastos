...
<body>
    ...
    <section class="tarjetas"> <!--TARJETAS PANEL DE CONTROL-->
        ...
        <section class="tarjeta">
            ...
            $sql_meta = "SELECT meta FROM movimientos WHERE id_user = '$id_user' ORDER BY id_movimiento DESC LIMIT 1";
            $result_meta = $conn->query($sql_meta);

            if ($conn->affected_rows > 0) {
                $row_meta = $result_meta->fetch_array();
                $_SESSION['meta'] = $row_meta['meta'];
            }

            if (isset($_SESSION['meta'])) {
                echo "<h1>$ {$row_meta['meta']}</h1>";
            } else {
        ...
        <section class="tarjeta">
            ...
            if (isset($_SESSION['meta'])) {
                $ahorro = ($_SESSION['total'] - $row_meta['meta']) * 10 / 100;
                echo "<h1>$ $ahorro</h1>";
            } else {
